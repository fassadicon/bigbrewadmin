    ...
    <div class="p-4 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        <div class="bg-white rounded-lg shadow-md p-6">
            ...
            @foreach ($lowInventoryItems as $lowInventoryItem)
                <p class="text-sm mb-2">{{ $lowInventoryItem->name }} - Remaining Stocks: {{ $lowInventoryItem->remaining_stocks }}</p>
            @endforeach
        </div>
        ...
        <div class="bg-white rounded-lg shadow-md p-6">
            ...
            @foreach ($mostOrderedProducts as $mostOrderedProduct)
                <p class="text-sm mb-2">{{ $mostOrderedProduct->productDetail->name }} - Order Items Count: {{ $mostOrderedProduct->order_items_count }}</p>
            @endforeach
        </div>

        <!-- Livewire Column Chart Card -->
        <div class="bg-white rounded-lg shadow-md p-6 col-span-3 md:col-span-2">
            <h2 class="text-lg mb-4">Livewire Column Chart</h2>
