<x-modal name="show-product-size">
    <div wire:loading.class="invisible">
        @if ($size)
            <div style="padding: 20px;">

                <h1 style="font-size: 24px; margin-bottom: 10px;">{{ $size->name }}</h1>
                <p style="font-size: 16px; margin-bottom: 10px;">{{ $size->measurement }}</p>
                <p style="font-size: 16px; margin-bottom: 20px;">{{ $size->description }}</p>

                <h2 style="font-size: 18px; margin-bottom: 10px;">Products with this size:</h2>

                <ul style="list-style-type: none; padding: 0; margin: 0;">
                    @forelse ($size->productDetails as $product)
                        <li style="font-size: 14px; margin-bottom: 5px;">{{ $product->name }}</li>
                    @empty
