        ...
        <div class="flex">
            <div class="relative w-full">
                ...
                <input wire:model.live.debounce.300ms='search' type="text"
                    ...
                    placeholder="Search name or description" required="">
            </div>
        </div>
    ...
    <div class="overflow-x-auto">
        <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
            ...
            <tbody wire:loading.class="invisible">
                @if (count($productDetails) < 1) <tr class="border-b dark:border-gray-700">
                    <td colspan="6" class="px-4 py-3 text-center">No results available</td>
                    </tr>
                    @else
                    @foreach ($productDetails as $productDetail)
                    <tr wire:key="{{ $productDetail->id }}" class="border-b dark:border-gray-700">
                        ...
                        <td class="px-4 py-3 flex items-center justify-center">
                            {{-- <x-nav-link :href="route('createProduct')"
                                :active="request()->routeIs('product-create')" wire:navigate>
                                {{ __('Create Product') }}
                            </x-nav-link> --}}
                            @if(auth()->user()->hasRole('Owner') || auth()->user()->hasRole('Admin'))
                            <a href="{{ route('products.edit', ['productDetail' => $productDetail]) }}" wire:navigate
                                class="p-2 mx-1 bg-green-500 text-white rounded">
                            ...
                            <button wire:click='delete({{ $productDetail }})'
                                class="p-2 mx-1 bg-orange-500 text-white rounded">
